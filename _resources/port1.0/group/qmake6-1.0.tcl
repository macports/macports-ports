...
pre-configure {
    ...
    if { [tbool qt6.add_spec] } {
        if {[vercmp ${qt6.version} >= 5.9]} {
            # configure.args-append "${qt6.spec_cmd}${qt_qmake_spec}"
            configure.args-append "${qt6.spec_cmd}macx-clang"
        } else {
            if {[variant_exists universal] && [variant_isset universal]} {
                global merger_configure_args
                lappend merger_configure_args(i386)   {*}${qt6.spec_cmd}${qt_qmake_spec_32}
                lappend merger_configure_args(x86_64) {*}${qt6.spec_cmd}${qt_qmake_spec_64}
            } else {
                # configure.args-append "${qt6.spec_cmd}${qt_qmake_spec}"
                configure.args-append "${qt6.spec_cmd}macx-clang"
            }
    ...
    if { [option configure.ccache] && [vercmp ${qt6.version} >= 5.9.2]} {
    ...
    if { [vercmp ${qt6.version} >= 5.6]} {
        ...
        if {${qmake6_cxx11_flags} ne ""} {
            puts ${cache} QMAKE_CXXFLAGS_CXX11+="${qmake6_cxx11_flags}"
        }
    } else {
        # always use the same standard library
