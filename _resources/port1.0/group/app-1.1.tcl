...
# The key can contain up to 15 characters; if you need longer string use
# app.display_name.
#
...
# app.display_name: the user-visible name for the bundle, used also by Siri
#
# The default is based on ${app.name}. There is no limit on size.
#
# Info.plist key CFBundleDisplayName.

options app.display_name
default app.display_name {${app.name}}
...
platform macosx {
    pre-destroot {
        if {[tbool app.create]} {
            ...
            # Ensure app.display_name is valid.
            if {[regexp {[/]} ${app.display_name}]} {
                return -code error "app.display_name ${app.display_name} contains illegal characters"
            }
        ...
        if {[tbool app.create]} {
            ...
            if {[file exists ${destroot}[app._resolve_symlink ${executable} ${destroot}]]} {
                if {[tbool app.use_launch_script]} then {
                    ...
                    # to comply with the new privacy control access introduced with macOS 10.14
                    # we need to move the binary to the .app and replace the original binary with a link
                    xinstall ${destroot}${executable} ${destroot}${applications_dir}/${app.name}.app/Contents/MacOS/${app.name}
                    delete ${destroot}${executable}
                    ln -s ${applications_dir}/${app.name}.app/Contents/MacOS/${app.name} ${destroot}${executable}
                }
                ...
                # Assume it belongs to a dependency and use it as the target.
                    ...
                    if {[tbool app.use_launch_script]} then {
                        ...
                        xinstall ${executable} ${destroot}${applications_dir}/${app.name}.app/Contents/MacOS/${app.name}
                        delete ${executable}
                        ln -s ${applications_dir}/${app.name}.app/Contents/MacOS/${app.name} ${executable}
                    }
                }
            } else {
...
<dict>
    ...
    <string>${app.display_name}</string>
